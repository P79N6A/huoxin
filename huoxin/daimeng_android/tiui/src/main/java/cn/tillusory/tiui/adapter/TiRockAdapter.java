 ...
 */
public class TiRockAdapter extends RecyclerView.Adapter<TiViewHolder> {

    private TiRockEnum[] list;

    private int selectedPosition = 0;
    private TiSDKManager tiSDKManager;

    public TiRockAdapter(TiRockEnum[] list, TiSDKManager tiSDKManager) {
        this.list = list;
    ...
    public void onBindViewHolder(@NonNull final TiViewHolder holder, int position) {
        ...
        if (selectedPosition == position) {
            ...
            tiSDKManager.setRockEnum(list[position]);
        } else {
